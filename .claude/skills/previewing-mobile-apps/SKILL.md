...
```bash
mobai-cloud preview run --detach --json                              # the app's own entry
mobai-cloud preview run --detach --entry Sources/Screens/GalleryView.swift --json
mobai-cloud preview run --detach --entry GalleryView --json          # SwiftUI: by view name
```
...
| `PREVIEW_UNSUPPORTED_MODULE` | a dependency cannot run in the preview | first check the target is the screen's file and not the project (see [Choosing what to preview](#choosing-what-to-preview)); if the screen really needs it, write an adapter: [writing-preview-adapters.md](./writing-preview-adapters.md) |
