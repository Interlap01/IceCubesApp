...
`ready`. Run setup again after installing anything, since some rows (the
...
2. The fonts, behind Apple's license gate (below). Without SF Pro the host
...
`Image(systemName:)` as a placeholder box. Each line names its fix.
The font fixes are a license gate (Apple's font and SF Symbols terms): that
...
silently settle for the degraded default on their behalf. Installing the
host's system packages needs no such decision.

The setup plan lists this as `apple-design-resources`, marked `optional`:
